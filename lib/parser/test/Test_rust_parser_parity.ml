...
let resolve_rust_parser_path () =
  ...
  in
  Support.resolve_binary
    ~env_var: "FORESTER_RUST_PARSER_PATH"
...
let check_node_span
  ...
  in
  let Lsp.Types.Position.{line = actual_start_line; character = actual_start_character}
    =
    start
  in
  let Lsp.Types.Position.{line = actual_end_line; character = actual_end_character}
    =
    end_
  in
...
let test_rust_bridge_preserves_spans () =
  with_rust_parser_or_skip "rust parser bridge preserves spans"
    ...
    match Rust_parser.parse_tree
      ~source_path
      {|\title{Hello}
\p{}|} with
    | Error errors ->
      Alcotest.failf
        "Rust parser span fixture failed:\n%s"
        (render_rust_errors errors)
    | Ok {
    ...
    | Ok {Code.code; _} ->
      Alcotest.failf
        "Expected four top-level nodes in span fixture, got %d"
        (List.length code)
...
let test_rust_bridge_surfaces_structured_errors () =
  with_rust_parser_or_skip "rust parser bridge surfaces structured errors"
    ...
    | Ok code ->
      Alcotest.failf
        "Expected mismatched delimiter parse failure, got %d nodes"
        (List.length code)
    | Error [error] ->
      (
...
\p{tail}|} with
    | Rust_parser.Recovered (code, errors) ->
      ...
      Alcotest.(check bool)
        "third node remains following paragraph ident"
        true
        (
    ...
    | Rust_parser.Parsed code ->
      Alcotest.failf
        "Expected recovered parse result, got parsed with %d nodes"
        (List.length code)
    | Rust_parser.Failed errors ->
      Alcotest.failf
        "Expected recovered parse result, got failure:\n%s"
        (render_rust_errors errors)
...
let test_rust_bridge_reports_header_binder_error_ranges () =
  with_rust_parser_or_skip
    ...
    | Error errors ->
      Alcotest.failf
        "Expected single header binder error, got %d"
        (List.length errors)
  ...
  run
    ...
    [
      (
        ...
        test_case
          "bridge preserves spans"
          `Quick
          test_rust_bridge_preserves_spans;
        test_case
          "bridge surfaces structured errors"
          `Quick
          test_rust_bridge_surfaces_structured_errors;
        test_case
          "bridge supports recovery mode"
          `Quick
          test_rust_bridge_supports_recovery_mode;
        test_case
          "bridge reports header binder error ranges"
          `Quick
