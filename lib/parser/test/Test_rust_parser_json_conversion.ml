...
let span_json ~start_offset ~start_line ~start_column ~end_offset ~end_line
    ~end_column =
  obj
    [
      ( "start",
...
let test_parse_outcome_of_json_decodes_all_node_variants () =
  match
    Rust_parser.parse_outcome_of_json "ignored input"
      (exhaustive_document_json ())
  with
  | Rust_parser.Parsed decoded_code ->
      Alcotest.(check Testables.code)
        "decoded exhaustive node set"
        (exhaustive_expected_code ()) decoded_code
  | Rust_parser.Recovered (_, errors) ->
      Alcotest.failf "Expected Parsed outcome, got Recovered with %d errors"
        (List.length errors)
  | Rust_parser.Failed errors ->
      Alcotest.failf "Expected Parsed outcome, got Failed with %d errors"
        (List.length errors)

let test_located_node_of_json_preserves_spans () =
  let source = Rust_parser.source_of_input ~source_path:"synthetic.tree" "hello" in
  let span =
    span_json ~start_offset:0 ~start_line:1 ~start_column:1 ~end_offset:5
...
let test_parse_outcome_of_json_handles_recovered_and_unknown_status () =
  ...
  | Rust_parser.Recovered (decoded_code, [ error ]) ->
      Alcotest.(check Testables.code) "recovered code" [ code_text "tail" ]
        decoded_code;
      Alcotest.(check string) "recovered error message" "synthetic error"
        error.message;
      Alcotest.(check int) "recovered start_offset" 1 error.start_offset;
      Alcotest.(check int) "recovered end_offset" 2 error.end_offset;
      Alcotest.(check string) "recovered report" "Error: synthetic error"
        error.report;
  ...
  | Rust_parser.Failed errors ->
      Alcotest.failf "Expected Recovered outcome, got Failed with %d errors"
        (List.length errors));
  match
    Rust_parser.parse_outcome_of_json "ignored"
      (obj [ ("status", str "mystery"); ("errors", list []) ])
  with
  | Rust_parser.Failed [ error ] ->
      Alcotest.(check string) "unknown status message"
        "Unknown status: mystery" error.message
  | Rust_parser.Failed errors ->
      Alcotest.failf "Expected one unknown-status error, got %d"
        (List.length errors)
  | Rust_parser.Parsed _ | Rust_parser.Recovered _ ->
      Alcotest.fail "Expected unknown status to decode as Failed"

let () =
  Alcotest.run "Rust parser JSON conversion"
    [
      ( "json-conversion",
        [
          Alcotest.test_case "decodes all node variants" `Quick
            test_parse_outcome_of_json_decodes_all_node_variants;
          Alcotest.test_case "preserves span ranges" `Quick
            test_located_node_of_json_preserves_spans;
          Alcotest.test_case "maps unknown node types to Code.Error" `Quick
            test_unknown_node_type_maps_to_code_error;
          Alcotest.test_case "handles recovered and unknown statuses" `Quick
