...
|}
  ...
  | Error [ error ] ->
      expect_contains ~label:"exit code included" ~needle:"exited with code 17"
        error.message;
      expect_contains ~label:"stderr included" ~needle:"stderr:\nfatal stderr"
        error.message;
...
let test_parse_reports_invalid_json_cleanly () =
  ...
  | Error [ error ] ->
      expect_contains ~label:"invalid json message" ~needle:"Parse error:"
        error.message
  | Error errors ->
      Alcotest.failf "Expected one invalid-json error, got %d"
        (List.length errors)

let () =
  Alcotest.run "Rust parser process"
    [
      ( "process",
        [
          Alcotest.test_case "availability uses exit status" `Quick
            test_is_available_accepts_exit_zero_without_stdout;
          Alcotest.test_case "unavailable parser fallback" `Quick
            test_parse_reports_unavailable_binary_predictably;
          Alcotest.test_case "exit code and stream classification" `Quick
            test_parse_surfaces_exit_code_and_split_streams;
          Alcotest.test_case "timeout handling" `Quick
            test_parse_times_out_and_returns_predictable_error;
          Alcotest.test_case "invalid json classification" `Quick
