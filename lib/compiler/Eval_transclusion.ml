...
let eval_subtree ~loc ~flags ~config
    ~(parent_frontmatter : T.content T.frontmatter) ~addr_opt ~nodes
    ~eval_tree_inner ~emit_tree ~emit_content_node =
  let uri =
    match addr_opt with
...
let latex_artefact ~loc ~config ~preamble ~body =
  let source =
    ...
    LaTeX_template.to_string ~document_class:latex.document_class
      ~document_class_options:latex.document_class_options ~preamble ~body
  in
