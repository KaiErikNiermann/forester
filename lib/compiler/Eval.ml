...
let get_transclusion_flags ~loc =
  Transclusion.get_transclusion_flags
    ~loc
    ~dynenv: (Dyn_env.read ())
    ~extract_bool
...
and eval_node node : Value.t =
  ...
  | Subtree (addr_opt, nodes) ->
    Transclusion.eval_subtree
      ~loc
      ~flags: (get_transclusion_flags ~loc)
      ~config: (Config_env.read ())
      ~parent_frontmatter: (Frontmatter.get ())
      ~addr_opt
      ~nodes
      ~eval_tree_inner
    ...
    begin
      ...
      | v' ->
        Reporter.fatal
          ...
          ~extra_remarks: [
            Asai.Diagnostic.loctextf
              "Expected solitary node but got %a / %a"
              Value.pp
              v
              Value.pp
              v';
        ...
        begin
          ...
          | _ ->
            Reporter.fatal
              ...
              ~extra_remarks: [
                Asai.Diagnostic.loctextf
                  "Expected %i additional arguments"
                  (List.length xs);
              ]
        end
    )
