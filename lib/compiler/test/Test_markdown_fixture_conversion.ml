...
let with_temp_markdown content f =
  Support.with_temp_file
    ~prefix: "forester_markdown_fuzz_"
    ~suffix: ".md"
    content
    f
...
let rec normalize_bridge_shape (code : Code.t) : Code.t =
  ...
  in
  let normalize_node
      ({Asai.Range.value; loc}: Code.node Asai.Range.located)
    =
...
let assert_well_formed_verbatim ~fixture output =
  ...
  if find_substring output "\\verb<|" 0 <> None then
    Alcotest.failf
      "Fixture %s contains known bad verbatim boundary token \\verb<|"
      fixture
  else loop 0
  ...
  run
    ...
    [
      (
        ...
        test_case
          "dual parser equivalence"
          `Quick
          test_dual_parser_equivalence;
        test_case
          "randomized markdown generation"
          `Quick
