...
let test_scan_directories_includes_markdown_sources ~env () =
  ...
  Alcotest.(check (list string))
    "markdown and tree files are scanned" expected scanned
...
let () =
  ...
  run "Dir_scanner"
    [
      ( "scan",
        [
          test_case "scan_directories includes markdown sources" `Quick
            (test_scan_directories_includes_markdown_sources ~env);
          test_case "find_tree resolves markdown basenames" `Quick
