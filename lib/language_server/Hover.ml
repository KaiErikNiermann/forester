...
let compute ({position; textDocument; _}: L.HoverParams.t) =
  ...
  let@ content =
    Option.map @~
      ...
      | None ->
        Reporter.fatal Internal_error ~extra_remarks: [Asai.Diagnostic.loctextf "%a is not in the index" URI.pp uri]
      | Some tree ->
  ...
  in
  L.Hover.create ~contents: (`MarkupContent {kind = L.MarkupKind.Markdown; value = content}) ()
