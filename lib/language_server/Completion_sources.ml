  ...
  let verb =
    let insert_text =
      Lsp.Snippet.(
        ...
        to_string @@ "\\startverb<<|\n" @+ (tabstop 1 +@ "\n<<|"))
    in
    L.CompletionItem.create ~insertTextFormat:Snippet ~insertText:insert_text
...
let asset_completions ~(config : Config.t) =
  ...
  let@ insert_text =
    ...
    @~ List.find_map
         ...
         asset_dirs
  in
  L.CompletionItem.create ~label:insert_text ~kind:File ~insertText:insert_text
    ()
...
let items_for_completion ~(forest : State.t) ~(position : L.Position.t)
    ~(completion : completion) ~(code : Tree.code option) =
