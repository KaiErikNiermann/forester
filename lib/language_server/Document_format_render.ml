...
and pp_methods_block ~render ~config ~indent fmt methods =
  List.iter
    (fun (name, body) ->
      ...
      pp_body ~render ~config ~indent:(indent + 1) ~closing_indent:method_indent
        fmt body;
      Format.pp_print_char fmt '}';
      Format.pp_print_newline fmt ())
    methods

and pp_node ~render ~config ~indent fmt
...
and pp_inline_node ~render ~config fmt
    ({ Range.value; _ } : Code.node Range.located) =
  match value with
  | Code.Text s -> Format.pp_print_string fmt s
