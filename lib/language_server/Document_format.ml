...
let pp_binding fmt (info, name) =
  (* The formatter works from the desugared AST, so it intentionally
     canonicalizes all header binders back to square-bracket core syntax. *)
  match info with
...
let is_block_node (node : Code.node) =
  match node with
  | Code.Subtree _ | Code.Def _ | Code.Let _ | Code.Namespace _ | Code.Object _
  | Code.Patch _ | Code.Scope _ | Code.Import _ | Code.Decl_xmlns _
  | Code.Comment _ ->
      true
  ...
  let frontmatter, body =
    let rec split_frontmatter acc = function
      ...
      | ({ Range.value; _ } as node) :: rest ->
          if is_frontmatter_node value then split_frontmatter (node :: acc) rest
          else begin
            match value with
            | Code.Comment _ ->
...
and pp_node ~config ~indent fmt ({ Range.value; _ } : Code.node Range.located) =
  ...
  | Code.Group (delim, inner) ->
      ...
      if is_simple_content inner then pp_inline_nodes ~config fmt inner
      else begin
        Format.pp_print_newline fmt ();
  ...
  | Code.Xml_ident (prefix, name) -> begin
      match prefix with
  ...
  | Code.Let (path, bindings, body) ->
      ...
      Format.pp_print_char fmt '{';
      if is_simple_content body then pp_inline_nodes ~config fmt body
      else begin
        Format.pp_print_newline fmt ();
  ...
  | Code.Object { self; methods } ->
      ...
      begin match self with None -> () | Some s -> Format.fprintf fmt "[%s]" s
      end;
      Format.pp_print_char fmt '{';
      Format.pp_print_newline fmt ();
      List.iter
        (fun (name, body) ->
          let method_indent =
            String.make ((indent + 1) * config.indent_width) ' '
          in
  ...
  | Code.Import (visibility, target) ->
      let cmd =
        match visibility with Private -> "import" | Public -> "export"
      in
...
and pp_inline_node ~config fmt ({ Range.value; _ } : Code.node Range.located) =
  ...
  | Code.Verbatim s ->
      if String.contains s '|' then Format.fprintf fmt "\\verb!%s!" s
      else Format.fprintf fmt "\\verb|%s|" s
  | Code.Comment s -> Format.fprintf fmt "%% %s" s
  | Code.Group (delim, inner) ->
      pp_delim_open fmt delim;
